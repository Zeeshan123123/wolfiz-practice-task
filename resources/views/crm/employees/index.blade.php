    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                @if(count($employees))
                    ...
                    <x-table>
                        <x-slot name="header">
                            ...
                            <x-table-col>Phone</x-table-col>
                            <x-table-col>Company</x-table-col>
                            <x-table-col>Status</x-table-col>
                            <x-table-col>Actions</x-table-col>
                        </x-slot>
                        @foreach($employees as $employee)
                            <tr>
                                <x-table-col>{{ $employee->id }}</x-table-col>
                                <x-table-col><a href="{{ route('employees.show', $employee) }}">{{ $employee->first_name }}</a></x-table-col>
                                <x-table-col>{{ $employee->last_name }}</x-table-col>
                                <x-table-col>{{ $employee->email }}</x-table-col>
                                <x-table-col>{{ $employee->phone }}</x-table-col>
                                <x-table-col>{{ $employee->company->name }}</x-table-col>
                                <x-table-col>{{ $employee->status }}</x-table-col>
                                <x-table-col>
                                    <form class="inline-flex items-center" action="{{ route('employees.destroy', $employee) }}" method="POST">
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button class="bg-indigo-50 ml-2 hover:bg-red text-grey-darkest font-bold py-2 px-4 rounded inline-flex items-center" onclick="return confirm('Do you want to delete the employee?')">
                ...
                @else
                    <p class="p-6 text-center">No Employee to show yet</p>
                @endif
            </div>
        </div>
    </div>
